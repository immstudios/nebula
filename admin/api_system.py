...
def api_meta_types(user, **kwargs):
    return 200, [meta_types[t].pack() for t in meta_types.keys()]


def api_site_settings(user, **kwargs):
    db = DB()
    rights = user.meta
...
def api_services(user, **kwargs):
    command    = kwargs.get("command", 0)
    id_service = kwargs.get("id_service", 0)
    db = DB()
...
def api_message(user, **kwargs):
    if kwargs.get("message", False):
        messaging.send("message", sender=auth_key, from_user=user["full_name"] or user["login"] , message=kwargs["message"])
    return 200, "ok"
