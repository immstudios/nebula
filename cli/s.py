...
def show_services(db):
    db.query(
        "SELECT id, service_type, host, title, autostart, state, last_seen FROM services ORDER BY id"
    )
    print()
    ...
    for id, stype, host, title, auto, state, last_seen in db.fetchall():
        ...
        data = {
            ...
            "warning": "NOT RESPONDING FOR {}".format(s2words(last_seen_age))
            if last_seen_age > 60
            else "",
        }
        print(
            "{id:<4}{type:<12}{title:<20}{host:<15}{auto:<6}{state} {warning}".format(
                **data
            )
        )
...
def s(*args):
    ...
    if len(args) >= 2:
        ...
        if args[0] == "start":
            db.query(
                "UPDATE services SET state=2 WHERE id IN %s AND state=0", [services]
            )
            db.commit()
        elif args[0] == "stop":
            db.query(
                "UPDATE services SET state=3 WHERE id IN %s AND state=1", [services]
            )
            db.commit()
        elif args[0] == "kill":
            db.query(
