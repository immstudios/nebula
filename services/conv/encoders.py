...
class NebulaFFMPEG(BaseEncoder):
        ...
        for p in self.task:
            if p.tag == "param":
                ...
                if not storages[id_storage]:
                    return NebulaResponse(500, message=f"Target storage {id_storage} is not mounted")
                ...
                if not temp_path:
                    return NebulaResponse(500, message="Unable to create temp directory")
                ...
                if not os.path.isdir(target_dir):
                    try:
                        ...
                        log_traceback()
                        return NebulaResponse(500, message=f"Unable to create output directory {target_dir}")
    ...
    def finalize(self):
        if self.proc.return_code > 0:
            ...
            return NebulaResponse(500, message=f"Encoding failed\n{self.proc.error_log}")
